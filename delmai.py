...
class DelMail():
    ...
    def decide(self):
        ...
        while True:
            ...
            if mails.lower() == 'yes' or mails.lower() == 'y':
                return self.marks
            elif mails.lower() == 'no' or mails.lower() == 'n':
                self.retopic()
                break
            else:
                print('请重新输入正确选项.\n')
    ...
    def retopic(self):
        ...
        while True:
            topic = input('请输入查询条件,eg:SINCE 01-May-2017:\n')
            try:
    ...
    def delemail(self, topic='ALL'):
        ...
        while True:
            ...
            if choince.lower() == 'yes' or choince.lower() == 'y':
                topic = input('\n请输入查询条件,eg:BEFORE 01-May-2017:\n')
                try:
                    self.marks = self.mail.search(topic)
                    print('\n符合查询条件的邮件有%s封。' % len(self.marks))
                    self.decide()
                    ...
                    #self.decide()
                    break
            elif choince.lower() == 'no' or choince.lower() == 'n':
                topic = 'ALL'
                self.marks = self.mail.search(topic)
                print('\n需要删除的邮件有%s封.\n' % (self.mailnums[b'EXISTS']))
                break
        ...
        while True:
            last = input('\n(慎重)是否进行邮件删除?(yes/no)\n')
            if last.lower() == 'yes'or last.lower() == 'y':
                ...
                os.system('pause')
                break
            elif last.lower() == 'no' or last.lower() == 'n':
                self.mail.logout()
                print('\n取消删除邮件,正在退出邮箱...')
                time.sleep(1)
                print('Bye,Bye\n')
